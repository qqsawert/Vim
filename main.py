...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir:
            checkpoint_file_name = 'checkpoint_batch_size_'
            checkpoint_file_name += str(args.batch_size)
            checkpoint_file_name += '.pth'
            ...
            for checkpoint_path in checkpoint_paths:
                utils.save_on_master({
                    ...
                    'max_gpu_mem': train_stats["max_gpu_mem"]
                }, checkpoint_path)
        ...
        if max_accuracy < test_stats["acc1"]:
            ...
            if args.output_dir:
                best_checkpoint_file_name = 'best_checkpoint_'
                best_checkpoint_file_name += str(args.batch_size)
                best_checkpoint_file_name += '.pth'
                checkpoint_paths = [output_dir / best_checkpoint_file_name]
                for checkpoint_path in checkpoint_paths:
                    utils.save_on_master({
                        'model': model_without_ddp.state_dict(),
                        'optimizer': optimizer.state_dict(),
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     **{f'test_{k}': v for k, v in test_stats.items()},
                     ...
                     'max_gpu_mem': train_stats.get('max_gpu_mem', None)
                    }
